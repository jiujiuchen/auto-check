...
(function($, win, doc, U){
    ...
    U.slider = function(selection, options){
        ...
        _t.containerDom = _t.sliderDom.find('.sliderContainer');
        _t.dragDom = _t.sliderDom.find('.sliderDrag');
        _t.barDom = _t.sliderDom.find('.bar');
        _t.blockDom = _t.sliderDom.find('.sliderBlock');
        _t.unitDom = _t.sliderDom.find('.sliderUnit');
        _t.inputDom = _t.sliderDom.find('.uc-input');
        ...
        _t.initUnit = function(){
           ...
           for(var i = 0, len = Math.ceil(_t.options.max/_t.options.step); i < len; i++){
			 _t.blockWidth.push(_t.maxWidth/len);
		   }
        ...
        _t.valueToIndex = function(v){
           ...
           var length = _t.blockDom.length;
            for(; i < length; i++){
				var blockDomText = _t.blockDom[i].getAttribute("value");
                if(val <= blockDomText){
                    idx = i;
                    break;
                }
        ...
        _t.barDom.click(function(ev){
			...
			_t.currentIndex  = _t.valueToIndex(_t.currentValue);
